...
partition(int nodenum, pNode splitnode, double *sumrisk, int n1, int n2,
          int minsize, int split_Rule, double alpha,  int bucketnum, int bucketMax, //double eta,
...
{Rprintf("partition\n");
   ...
   //R_ShowMessage("R_show_message\n");
    ...
    if (nodenum > 1) {
	    ...
	    if (split_Rule == 1) {
          ...
          &(me->risk), ct.wtemp, ct.trtemp, ct.max_y, ct.propensity);
	        ...
	        // ct
	        (*ct_eval) (n, ct.ytemp, me->response_est, me->controlMean, me->treatMean, 
          &(me->risk), ct.wtemp, ct.trtemp, ct.trstemp, ct.max_y, alpha,  train_to_est_ratio); //eta,
    ...
    bsplit(me, n1, n2, min_node_size, split_Rule, alpha,  bucketnum, bucketMax, train_to_est_ratio); //eta,
 ...
 /*   print_tree(me, 4); */
    ...
    left_split = partition(2 * nodenum, me->leftson, &left_risk, n1, n1 + nleft,
                           min_node_size, split_Rule, alpha,  bucketnum, bucketMax, //eta,
                           train_to_est_ratio);
